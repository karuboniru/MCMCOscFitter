...
public:
  MinuitFitter(ParBinnedInterface &binned_interaction_, SimpleDataHist &data_,
               double param::*param_ptr_, param param_,
               std::vector<double> ebin_edges_,
               std::vector<double> costh_bin_edges_)
      : binned_interaction(binned_interaction_), data(data_),
        param_ptr(param_ptr_), par(param_), ebin_edges(ebin_edges_),
        costh_bin_edges(costh_bin_edges_) {}
...
int main(int argc, char **agrv) {
  ...
  for (auto &&[var, min, max] : inputs) {
    MinuitFitter fitter_NH(bint, cdata_NH_rebinned, var, par_IH, e_bin_wing,
                           costh_bin_wing);
    MinuitFitter fitter_IH(bint, cdata_IH_rebinned, var, par_NH, e_bin_wing,
                           costh_bin_wing);

    double bestChi2_NH = std::numeric_limits<double>::infinity();
    double bestParam_NH = 0.0;
    double bestChi2_IH = std::numeric_limits<double>::infinity();
    double bestParam_IH = 0.0;

    for (int i = 0; i < 64; ++i) {
      ROOT::Minuit2::MnUserParameters fitparNH{};
      fitparNH.Add("var", (min + max) / 2 * get_random(), (max - min) / 500,
                   min, max);
      auto resultNH = ROOT::Minuit2::MnMigrad{fitter_NH, fitparNH}();
      if (resultNH.Fval() < bestChi2_NH) {
        bestChi2_NH = resultNH.Fval();
        bestParam_NH = resultNH.UserParameters().Params()[0];
      }

      ROOT::Minuit2::MnUserParameters fitparIH{};
      fitparIH.Add("var", (min + max) / 2 * get_random(), (max - min) / 500,
                   min, max);
      auto resultIH = ROOT::Minuit2::MnMigrad{fitter_IH, fitparIH}();
      if (resultIH.Fval() < bestChi2_IH) {
        bestChi2_IH = resultIH.Fval();
        bestParam_IH = resultIH.UserParameters().Params()[0];
      }
    }

    if (var == &param::DM2) {
      std::println("DM2: Best fit (NH): var={:.2e}, chi2={:.3f}, Best fit "
                   "(IH): var={:.2e}, chi2={:.3f}",
                   bestParam_NH, bestChi2_NH, bestParam_IH, bestChi2_IH);
    } else if (var == &param::T23) {
      std::println("T23: Best fit (NH): var={:.3f}, chi2={:.3f}, Best fit "
                   "(IH): var={:.3f}, chi2={:.3f}",
                   bestParam_NH, bestChi2_NH, bestParam_IH, bestChi2_IH);
    } else if (var == &param::DCP) {
      std::println("DCP: Best fit (NH): var={:.3f}pi, chi2={:.3f}, Best fit "
                   "(IH): var={:.3f}pi, chi2={:.3f}",
                   bestParam_NH / M_PI, bestChi2_NH, bestParam_IH / M_PI,
                   bestChi2_IH);
    }
  }
